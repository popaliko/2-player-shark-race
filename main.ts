sprites.onOverlap(SpriteKind.Player, SpriteKind.Finish, function (sprite, otherSprite) {
    carnival.customGameOverExpanded("Player " + mp.getPlayerProperty(mp.getPlayerBySprite(sprite), mp.PlayerProperty.Number) + " Wins!", effects.hearts, music.baDing, carnival.ScoreTypes.LTime)
})
...
let list: Image[] = []
list = [assets.image`p1`, assets.image`p2`]
scene.setBackgroundImage(assets.image`bg`)
set_up_for_players(2)
let finish = sprites.create(assets.image`finish`, SpriteKind.Finish)
finish.setPosition(150, 50)
game.wrap(function () {
    carnival.addLabelTo("Shark Race", carnival.Areas.Bottom)
    game.showLongText("P1, press the S button to Start", DialogLayout.Bottom)
